...
The **loop guard** in [`oq_power_house_strategy.yaml`](../openquatt/oq_power_house_strategy.yaml)
answers that by running one heat pump at compressor level 1 while the boiler is
drawing, which is what the official Quatt Full Electric does. Settings and
behaviour are documented in [Power House](power-house.md#lusbeveiliging-optioneel-fork);
what matters here is the wiring between the two features:
